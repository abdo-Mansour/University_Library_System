...
class AdminMenu(ttk.Frame):
    def __init__(self, parent, app, controller):
        ...
        message = Label(self, text="Hello, Admin! How may I serve you ?")
        message.grid(row=0, column=1, padx=10, pady=10)

        # Row 1
        browse_books_button = ttk.Button(
            self, text="Browse Books", command=self.browse)
        ...
        add_book_button = ttk.Button(
            self, text="Add Book", command=lambda: self.app.show_frame("AddBook"))
        ...
        add_book_button = ttk.Button(
            self, text="Delete User", command=lambda: self.app.show_frame("DeleteUser"))
        add_book_button.grid(row=2, column=2, padx=0, pady=10)

        # Row 3
        add_book_button = ttk.Button(
            self, text="Book Location", command=lambda: self.app.show_frame("BookLocation"))
        ...
        add_book_button = ttk.Button(
            self, text="Delete Book", command=lambda: self.app.show_frame("DeleteBook"))
        add_book_button.grid(row=3, column=2, padx=0, pady=10)

        # Row 4
        add_book_button = ttk.Button(
            self, text="Statistics (Report)", command=self.report)
        add_book_button.grid(row=4, column=1, padx=0, pady=10)

        # Row 5
        add_book_button = ttk.Button(
            self, text="Log Out", command=self.log_out)
